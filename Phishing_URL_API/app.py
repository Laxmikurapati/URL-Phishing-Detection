...
# url="http://yeniik.com.tr/wp-admin/js/login.alibaba.com/login.jsp.php"
...
def predict():
    try:
        ...
        url = data.get('url', '')  # Assuming the URL is 
        ...
        y_pred =loaded_model.predict(x)[0]
        if y_pred == 1:
            app.logger.info("Good prediction")
            return jsonify({'prediction': "good"})
        else:
            app.logger.info("Bad prediction")
            return jsonify({'prediction': "good"})
