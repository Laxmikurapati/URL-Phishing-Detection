...
// Declare predictedTabs as a global variable
// Declare predictedTabs as a global variable
const predictedTabs = new Set();
...
chrome.webNavigation.onBeforeNavigate.addListener(async function (details) {
    ...
    // Skip prediction and loading page for Chrome home page
    ...
    // Check if prediction is already performed for this tab
    if (predictedTabs.has(tabId)) {
        console.log('Prediction already made for tab:', tabId);
        return;
    }

    // Display loading screen while waiting for prediction
    ...
    try {
        ...
        // Use closure to capture the correct tabId value
        
            if (data.prediction === 'bad') {
                // Show the intermediate warning page
                const warningPage = chrome.runtime.getURL('warning.html') + `?url=${encodeURIComponent(url)}`;
                predictedTabs.add(tabId); // Mark this tab as 
                // Update to the warning page
                chrome.tabs.update(tabId, { url: warningPage });
            } else {
                // If prediction is good, perform additional actions
...
// Function to check if the URL is the Chrome home page
function isChromeHomePage(url) {
    return url.toLowerCase().startsWith('chrome://newtab/') || url === '' || url.toLowerCase().startsWith('chrome://homepage/') || url=="abuto:blank";
}
